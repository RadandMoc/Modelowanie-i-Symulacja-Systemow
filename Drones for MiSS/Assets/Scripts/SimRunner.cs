...
public class SimRunner : MonoBehaviour
{
	...
	private void SaveResult(string path, Dictionary<uint, double> fitnesses)
	{
		...
		{
			Debug.LogError($"[Worker {workerId}] Błąd podczas zapisu wyniku do {path}. Wyjątek: {ex.ToString()}");
		}
	}
	...
	private NeatGenome ReadGenome(string path)
	{
		...
		try
		{
			...
			{
				Debug.LogError($"[Worker {workerId}] Główny element XML to nie '{XmlElementNameNetwork}' lub jest null. Znaleziono: '{(genomeNode?.Name ?? "null")}'. Plik: {path}");
				Application.Quit();
			...
			Debug.Log($"[Worker {workerId}] Genom ID [{genome.Id}] wczytany. Neurony: {genome.NeuronGeneList.Count}, Połączenia: {genome.ConnectionGeneList.Count}. Wejścia: {genome.InputNeuronCount}, Wyjścia: {genome.OutputNeuronCount}");
			return genome;
		}
		catch (Exception ex)
		{
			Debug.LogError($"[Worker {workerId}] Krytyczny błąd podczas wczytywania/parsowania genomu z {path}. Wyjątek: {ex.ToString()}");
			Application.Quit();
			throw new ArgumentException();
		}
	}
	...
	private IBlackBox BlackBoxGenerator(NeatGenome genome)
	{
		...
		try
		{
			// KROK 1: Stworzenie IActivationFunctionLibrary (musi być identyczna jak w Trainerze)
			// Zakładamy, że Trainer używa: DefaultActivationFunctionLibrary.CreateLibraryNeat(new ReLU());
			...
			// Powinny być spójne z konfiguracją Trainera. Domyślnie FeedforwardOnly=true.
			NeatGenomeParameters neatGenomeParams = new NeatGenomeParameters();
			// Jeśli Trainer modyfikuje neatGenomeParams (np. neatGenomeParams.FeedforwardOnly = false),
			// należy to odzwierciedlić tutaj. Dla przykładu, załóżmy domyślne wartości.
			...
			// Używamy liczby wejść/wyjść z wczytanego genomu.
			...
			// To kluczowy krok, aby genome.ActivationFnLibrary było dostępne dla dekodera.
			...
			if (phenome == null)
			{
				Debug.LogError($"[Worker {workerId} | Genom {genome.Id}] KRYTYCZNY BŁĄD: Fenotyp jest null po dekodowaniu. Genom może być nieprawidłowy lub dekoder zawiódł.");
				Application.Quit();
				throw new ArgumentException();
			}
		...
		{
			Debug.LogError($"[Worker {workerId} | Genom {genome.Id}] KRYTYCZNY BŁĄD podczas tworzenia fenotypu (konfiguracja fabryki/dekodera). Wyjątek: {ex.ToString()}");
			Application.Quit();
			throw new ArgumentException();
		}
	}
